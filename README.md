...
演示程序基于STM32F411CEU6最小开发板Black Pill https://stm32-base.org/boards/STM32F411CEU6-WeAct-Black-Pill-V2.0 ，配置是100MHz主频、512k FLASH和128k RAM。
...
|   |- startup_stm32f411xe.s     主入口、ROM和插件接口地址
|   |- makefile                  编译脚本
|   └- STM32F411CEUx_FLASH.ld    链接脚本，定义FLASH、RAM范围
|- main                       主程序
|   |- build                     中间文件和编译好的二进制
|   |- Drivers                   依赖库
|   |- Inc                       
...
|   |- main_stm32f411.ioc        stm32cubemx项目文件
|   |- startup_stm32f411xe.s     主入口、中断向量
|   |- makefile                  编译脚本（警告：CUBE重新生成将会覆盖修改）
|   └- STM32F411CEUx_FLASH.ld    链接脚本（警告：CUBE重新生成将会覆盖修改）
|- plugin                     插件
|   |- bin                       编译好的二进制
|   |- obj                       编译中间文件
|   |- plugin.c                  插件接口实现
...
|bl/STM32F411CEUx_FLASH.ld|64～65行|定义bootloader使用RAM|
|bl/STM32F411CEUx_FLASH.ld|66～70行|定义bootloader及接口地址表使用的FLASH|
|main/STM32F411CEUx_FLASH.ld|64～65行|定义主程序使用RAM|
|main/STM32F411CEUx_FLASH.ld|66～68行|定义主程序及接口地址表使用的FLASH|
|plugin/STM32F411CEUx_FLASH.ld|63行|定义插件使用RAM|
|plugin/STM32F411CEUx_FLASH.ld|64～66行|定义插件及接口地址表使用的FLASH|
...
先后烧写主程序 main/build/main_stm32f411.elf 和引导程序 bl/bin/bl_stm32f411.elf，例如使用pyocd：
``` bash
pyocd flash -t stm32f411ce main/build/main_stm32f411.elf

pyocd flash -t stm32f411ce bl/bin/bl_stm32f411.elf
```
...
主程序烧写后，在烧写bootloader前，正常可以看到LED慢速闪烁（约0.5Hz）。
bootloader烧写后，在下载插件，正常可以看到LED快速闪烁（约4Hz），表示等待下载，此时主程序和引导程序是一起执行的。
...
5. 波特率选择115200 （非默认）
6. 点击Browse...
7. 选择plugin/bin/plugin_stm32f103.srec，注意只支持srec格式
8. 等待进度条满下载完成后程序自动退出
...
下载插件后，每次重启开发板，前5秒LED快速闪烁（4Hz），5秒后变得更快（10Hz），表示插件正常运行，调用主程序通信接口设置了参数。
